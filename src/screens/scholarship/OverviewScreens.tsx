...
export function ScholarshipEligibilityScreen({ navigation }: { navigation: Nav }) {
  ...
  return <Page refreshing={query.isRefetching} onRefresh={() => void query.refetch()} header={<Header title="Eligibility" subtitle="CURRENT POLICY EVALUATION" navigation={navigation} />}>
    {query.isLoading ? <Loading /> : data ? <>
      <View style={styles.card}><View style={styles.between}><View><Text style={styles.metric}>{Math.round(data.eligibility?.attendance.percentage ?? 0)}%</Text><Text style={styles.metricLabel}>CREDITED ATTENDANCE</Text></View><Badge value={data.eligibility?.eligible ? 'met' : data.eligibility?.needsReview ? 'review' : 'attention'} /></View><Text style={[styles.body, { marginTop: 14 }]}>{data.eligibility?.attendance.credited ?? 0} credited of {data.eligibility?.attendance.required ?? 0} required; {data.eligibility?.attendance.scheduled ?? 0} scheduled this quarter.</Text></View>
      {(data.eligibility?.checks ?? []).map((check) => <View key={check.key} style={styles.card}><View style={styles.between}><Text style={styles.actionTitle}>{check.label}</Text><Badge value={check.status} /></View></View>)}
      {(data.eligibility?.reasons ?? []).length ? <View style={styles.card}><Text style={styles.actionTitle}>Needs attention</Text>{data.eligibility?.reasons.map((reason) => <Text key={reason} style={[styles.body, { marginTop: 7 }]}>• {reason}</Text>)}</View> : null}
    ...
    <ActionLink icon="calendar-outline" title="Attendance plan" body="Record conflicts and your attendance commitment." onPress={() => navigation.navigate('ScholarshipAttendancePlan')} />
    <ActionLink icon="shield-checkmark-outline" title="Excuse requests" body="Submit or review a documented absence." onPress={() => navigation.navigate('ScholarshipAbsences')} />
    <ActionLink icon="podium-outline" title="Performance reviews" body="See six-month evaluation outcomes." onPress={() => navigation.navigate('ScholarshipPerformance')} />
  </Page>
}
