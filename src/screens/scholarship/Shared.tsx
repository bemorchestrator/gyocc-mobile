...
export function ActionLink({ icon, title, body, onPress, plain = false }: { icon: React.ComponentProps<typeof Ionicons>['name']; title: string; body: string; onPress: () => void; plain?: boolean }) {
  return <TouchableOpacity style={[styles.action, plain && styles.actionPlain]} activeOpacity={.72} onPress={onPress}><View style={styles.actionIcon}><Ionicons name={icon} size={21} color={color.maroon} /></View><View style={styles.grow}><Text style={styles.actionTitle}>{title}</Text><Text style={styles.actionBody}>{body}</Text></View><Ionicons name="chevron-forward" size={17} color={color.dim} /></TouchableOpacity>
}
